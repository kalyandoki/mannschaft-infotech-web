...
const getNavLinks = () => [
  ...
  { to: "industry-manufacturing", label: "Industry Manufacturing" },
  { to: "transportation-logistics", label: "Transportation Logistics" },
  { to: "healthcare", label: "Healthcare" },
  { to: "banks-insurance", label: "Banks & Insurance" },
  { to: "consulting-providers", label: "Consulting Providers" },
  { to: "non-profit", label: "Non Profit" },
];

const IndustryFocusHeader = () => {
  const {
    isMobileMenuOpen,
    toggleMenu,
    closeMenu,
        ...
        <nav className="hidden md:flex items-center gap-2 lg:gap-8 font-roboto">
          {navLinks.map(({ to, label }) => (
            <Link
              ...
              className={`cursor-pointer text-[#000] text-sm lg:text-md rounded-full transition duration-300  hover:text-[#010ED0] ${
                activeSectionHead === to ? "text-[#010ED0]" : ""
              }`}
            >
              {label}
            </Link>
...
export default IndustryFocusHeader;
